...
export default function HomeScreen() {
  ...
  const getSmartAIResponse = async (prompt: string) => {
    try {
      ...
      if (data?.weather && typeof data.weather.temp === 'number') {
        // Convert Celsius to Fahrenheit correctly
        const fahrenheit = (data.weather.temp * 9) / 5 + 32;
        return `The current weather in ${data.weather.city} is ${data.weather.description} with a temperature of ${fahrenheit.toFixed(1)}°F.`;
      }
